...
class KinodynamicRRT:
    def __init__(self, simulator, available_actions, t_max=10.0, rrt_dt=0.5, sim_dt=0.02):
        ...
        # 加重了姿态的权重，防止算法去选择处于极端姿态的节点展开
        self.weights = np.array([
            0.1, 0.1, 1.0,    
            0.5, 0.5, 0.5,    
            10.0, 10.0, 10.0, # 滚转、俯仰、偏航权重加大，逼它选平稳的父节点   
            1.0, 1.0, 1.0     
        ])
    ...
    def sample_random_state(self, current_best_pn):
        ...
        rand_state[1] = random.uniform(-20, 20) # 进一步收紧侧偏，逼它飞直线
        
        # 给它呼吸空间：引力目标在 2000m 到 2800m 游走，顺应它的起步爬升
        rand_state[2] = -random.uniform(2000, 2800)              
        rand_state[3] = random.uniform(230, 260) 
        rand_state[7] = 0.0 
        return rand_state
    ...
    def simulate_forward(self, start_state, action, duration):
        ...
        for _ in range(steps):
            ...
            # 【绝对物理红线】：严丝合缝按照你要求的 -5 到 15！
            if alpha > 15.0 or alpha < -5.0:
                return None 
            # 放宽高度死亡线，只要不跌落地面就行
            if h > 5000.0 or h < 1000.0:
                return None 
                
        return self.sim.state.copy()

    def generate_expert_trajectories(self, max_iter=2500, num_trajectories=1):
        root_node = {
        ...
        for i in range(max_iter):
            ...
            # 【黄金比例采样】：保底动作 + 7个随机动作 = 8条分支。防死锁最佳数量。
        ...
        def evaluate_leaf(n):
            ...
            # 【唯一的安全网】：只有翻肚皮倒飞才会没收成绩
            if abs(roll_deg) > 60.0:  
                return -float('inf')
                
            # 扣分项 1：绕圈惩罚
            drift_penalty = abs(pe) * 20.0 
            # 扣分项 2：高度惩罚（让它尽量待在 2000m~2500m，不要上天）
            alt_penalty = max(0, h - 2500.0) * 10.0 + max(0, 2000.0 - h) * 10.0
            # 扣分项 3：掉速惩罚
            speed_penalty = max(0, 230.0 - V) * 10.0
            # 扣分项 4：剧烈摇摆惩罚
            attitude_penalty = abs(phi) * 500.0 + abs(theta) * 100.0 + (abs(p) + abs(q) + abs(r)) * 1000.0

            return pn - drift_penalty - alt_penalty - speed_penalty - attitude_penalty
...
def plot_trajectory(trajectory):
    ...
    fig.suptitle('RRT 专家轨迹生成 (严守物理红线 + 软惩罚自适应)', fontsize=20, fontweight='bold', color='cyan')
    ...
    ax4_twin.axhline(y=-5, color='red', linestyle=':', alpha=0.5) 
...
if __name__ == "__main__":
    ...
    # 【起点回归】：回到我们最熟悉的 2000m，因为我们有完整的 debug 数据支撑
    ...
    for segment in range(total_segments):
        ...
        # 稳妥起见，给足 2500 次迭代，保证探索充分
        best_trajs = rrt_planner.generate_expert_trajectories(max_iter=2500, num_trajectories=1)
