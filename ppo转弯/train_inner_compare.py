...
# =================================================================
# 1.5 【对比实验核心】论文 (Yue et al., Aerosp. Sci. Technol. 2026) 奖励函数
#     —— 子目标分段奖励：跟踪项 f(e) + 舵速率惩罚 g(Δa) + 稳定子目标 h(rate, e)
#     已按本仿真器单位/包络做最小适配(角度用度、动作∈[-1,1]、dt=0.01)，
#     但保留论文的"分段+小误差子目标"内核，不与本人创新奖励混用。
# =================================================================
PAPER_CFG = dict(
    # f(e) 分段阈值(度)：|e|≤κ1 精跟踪子目标(高正奖励峰)；κ1<|e|≤κ2 正奖励线性衰减；|e|>κ2 有界指数惩罚
    KAPPA1=dict(theta=0.5, phi=0.5, beta=0.3),
    # κ2 按各轴指令包络设置：整个跟踪带内 f 恒正，给稠密向零梯度；指数惩罚区只在逼近终止边界时触发
    KAPPA2=dict(theta=15.0, phi=45.0, beta=12.0),
    EXP_ARG_CAP=2.5,   # 发散区指数自变量上限 → 惩罚有界(约 -11)，防回报量级失控
    F_FLOOR=-15.0,     # f 输出下限护栏
    # 论文舵速率限 50°/s，dt=0.01s、±25°标度 → 动作单位阈值 0.5/25 = 0.02
    DA_THRESH=0.02,
    # 稳定子目标 h 的门限：误差(度) 与 角速率(度/s)
    H_ERR_GATE=dict(theta=0.05, phi=0.05, beta=0.02),
    H_RATE_GATE=dict(theta=5.0, phi=5.0, beta=2.0),
    W1=0.01,          # 论文 ω1：舵速率惩罚(次要)
    W2=0.1,           # 论文 ω2：稳定子目标(次要)
    W3=0.1,           # 论文 ω3：方向轴额外滚转误差惩罚(仅 Eq.18)
    W3_PHI_REF=20.0,  # 方向轴滚转耦合惩罚归一化基准(度)：把不可控指令方差压成有界小量
)


def paper_track_f(e, k1, k2, arg_cap, floor):
    """论文分段跟踪奖励的"有界正奖励"重塑版(保留论文三区分段 + 小误差子目标内核)。
    - |e|≤κ1 精跟踪区：平滑正峰 exp(κ1-|e|)，原点导数不为零(论文消稳态误差的核心思想)；
    - κ1<|e|≤κ2 过渡区：正奖励从 1.0 线性衰减到 0(整个跟踪带恒正，给稠密向零梯度)；
    - |e|>κ2 发散区：有界指数惩罚(curb divergence)。三区在 κ1、κ2 处连续。
    与论文原式区别仅在"量级与正负偏置"，分段几何与子目标语义保持一致。"""
    ae = abs(e)
    if ae <= k1:
        val = math.exp(k1 - ae)               # 0→exp(κ1), κ1→1.0
    elif ae <= k2:
        val = (k2 - ae) / (k2 - k1)           # κ1→1.0, κ2→0.0
    else:
        val = -(math.exp(min(ae - k2, arg_cap)) - 1.0)
    return max(val, floor)


def paper_rate_g(da, thr):
    """论文舵面偏转速率惩罚 g(Δa)：阈值内线性、超阈指数；da=thr 处一阶连续。"""
    if da <= thr:
        return -da
    return -math.exp(da - thr) + (1.0 - thr)


def paper_stab_h(rate, err, err_gate, rate_gate):
    """论文稳定子目标 h(rate, e)：误差落入门限且角速率小时给附加奖励，抑制振荡。
    论文原式写作 (rate-1)^2 门控，但该式在 rate→0 方向给奖励反而偏小(对负速率给最大),
    与正文"抑制角速率振荡"语义相悖；此处按其叙述意图改为 rate→0 给最大奖励并归一化到[0,1]。"""
    if abs(err) <= err_gate and abs(rate) <= rate_gate:
        return 1.0 - (abs(rate) / rate_gate) ** 2
    return 0.0
...
class TrainingMetricsCallback(BaseCallback):
    ...
    def _on_training_end(self) -> None:
        ...
        fig.suptitle(f'X-47B 内环 {self.tag} 训练监控 (论文奖励对比组)', fontsize=15, fontweight='bold')
        ...
        save_dir = './logs_paper/metrics_plots/'
...
class X47BInnerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        controls = {
            'd_flap_L': np.clip(delta_e, -25.0, 25.0), 'd_flap_R': np.clip(delta_e, -25.0, 25.0),
            'd_ail_L': np.clip(delta_a, -20.0, 20.0), 'd_ail_R': np.clip(-delta_a, -20.0, 20.0),
            # 偏航：前扰流板(d_spoil_L)作带符号双向阻力舵；后扰流板数据弱，不用
            'd_spoil_L': np.clip(delta_r, -25.0, 25.0),
            'd_spoil_R': 0.0,
            'throttle': 0.65,
        }
        ...
        # ========== 对比实验：核心奖励 = 论文(Yue et al., 2026)子目标分段奖励 ==========
        # 仅替换"跟踪/舵速率/稳定"三项核心塑形；SURVIVE 生存低保与下方终止惩罚属
        # 共享环境脚手架(两组实验一致)，用于防止探索期自杀式提前终止、保证可训练性。
        d_net = abs(raw_act - self.prev_actions['net_out'])   # 论文 Δa = |a_t - a_{t-1}|(动作单位)
        K1, K2 = PAPER_CFG['KAPPA1'], PAPER_CFG['KAPPA2']
        CAP, FLOOR = PAPER_CFG['EXP_ARG_CAP'], PAPER_CFG['F_FLOOR']
        HE, HR = PAPER_CFG['H_ERR_GATE'], PAPER_CFG['H_RATE_GATE']
        W1, W2, W3 = PAPER_CFG['W1'], PAPER_CFG['W2'], PAPER_CFG['W3']
        reward = 0.0

        if self.stage == 1:
            # 方向轴(论文 Eq.18)：f(e_β) + ω1 g(Δa_r) + ω2 h(β̇, e_β) - ω3|e_φ|
            e_beta = self.hist_e_beta[0]
            e_phi = self.hist_e_phi[0]
            beta_dot = -(self.hist_e_beta[0] - self.hist_e_beta[1]) / self.dt  # target_β=0 → β=-e_β
            f = paper_track_f(e_beta, K1['beta'], K2['beta'], CAP, FLOOR)
            g = paper_rate_g(d_net, PAPER_CFG['DA_THRESH'])
            h = paper_stab_h(beta_dot, e_beta, HE['beta'], HR['beta'])
            # 滚转耦合惩罚归一化为有界小量：此阶段滚转由 PID 控、指令随机，
            # 原 -W3*|e_φ| 是不由本智能体动作决定的"纯噪声"，会淹没侧滑跟踪信号
            r_phi_couple = -W3 * min(abs(e_phi), PAPER_CFG['W3_PHI_REF']) / PAPER_CFG['W3_PHI_REF']
            reward = f + W1 * g + W2 * h + r_phi_couple + ENV_CFG['SURVIVE']
            self.prev_actions.update({'r': delta_r, 'a': delta_a, 'e': delta_e, 'net_out': raw_act})

        elif self.stage == 2:
            # 横滚轴(论文 Eq.17)：f(e_φ) + ω1 g(Δa_a) + ω2 h(p, e_φ)
            e_phi = self.hist_e_phi[0]
            f = paper_track_f(e_phi, K1['phi'], K2['phi'], CAP, FLOOR)
            g = paper_rate_g(d_net, PAPER_CFG['DA_THRESH'])
            h = paper_stab_h(p, e_phi, HE['phi'], HR['phi'])
            reward = f + W1 * g + W2 * h + ENV_CFG['SURVIVE']
            self.prev_actions.update({'a': delta_a, 'r': delta_r, 'e': delta_e, 'net_out': raw_act})

        elif self.stage == 3:
            # 俯仰轴(论文 Eq.15)：f(e_θ) + ω1 g(Δa_e) + ω2 h(q, e_θ)
            e_theta = self.hist_e_theta[0]
            f = paper_track_f(e_theta, K1['theta'], K2['theta'], CAP, FLOOR)
            g = paper_rate_g(d_net, PAPER_CFG['DA_THRESH'])
            h = paper_stab_h(q, e_theta, HE['theta'], HR['theta'])
            reward = f + W1 * g + W2 * h + ENV_CFG['SURVIVE']
...
def train_stage(stage, total_timesteps, n_envs, seed,
    ...
    metrics_cb = TrainingMetricsCallback(tag=f'stage{stage}_paper', verbose=1)
...
if __name__ == '__main__':
    ...
    torch.set_num_threads(1)
    n_envs = max(1, multiprocessing.cpu_count() - 5)

    print("=====================================================")
    print("  X-47B 飞翼内环【对比实验组】论文奖励函数 (Yue et al., 2026)")
    print("  与 train_innernew.py 仅奖励函数不同，其余完全一致")
    print(f"  进程数: {n_envs}")
    print("=====================================================")

    # ★ 对比组需用论文奖励重新训练全部三个 stage(模型名带 _paper，与本人实验隔离)
    # ★ 必须按 1→2→3 顺序训练：后阶段依赖前阶段已训练好的 _paper 模型
    # ---- Stage 1: 偏航/侧滑(差动扰流板) ----
    train_stage(stage=1, total_timesteps=1_200_000, n_envs=n_envs, seed=SEED,
                model_paths=None, best_dir='./logs_paper/best_model_stage1/',
                save_name='ppo_dir_stage1_paper', results_dir='./logs_paper/results_stage1/')

    # ---- Stage 2: 横滚(副翼) ----
    train_stage(stage=2, total_timesteps=1_200_000, n_envs=n_envs, seed=SEED,
                model_paths={'dir': 'ppo_dir_stage1_paper'}, best_dir='./logs_paper/best_model_stage2/',
                save_name='ppo_lat_stage2_paper', results_dir='./logs_paper/results_stage2/')

    # # ---- Stage 3: 俯仰(升降副翼) ----
    train_stage(stage=3, total_timesteps=1_400_000, n_envs=n_envs, seed=SEED,
                model_paths={'dir': 'ppo_dir_stage1_paper', 'lat': 'ppo_lat_stage2_paper'},
                best_dir='./logs_paper/best_model_stage3/',
                save_name='ppo_lon_stage3_paper', results_dir='./logs_paper/results_stage3/')

    # print("✅ 对比组三阶段内环训练完成，可与本人奖励组在相同评估脚本下对比 MAE/ITAE/Jδ。")